...
def get_all_medical_beds_of_given_type(
    ...
    return db.exec(
        select(MedicalBed)
        .where(col(MedicalBed.medical_bed_type_id) == medical_bed_type_id)
    ).all()
...
def get_medical_beds_of_hospital(
    ...
    return db.exec(
        select(MedicalBed)
        .where(col(MedicalBed.hospital_id) == hospital_id)
    ).all()
...
def get_medical_bed_by_id(
    ...
    medical_bed = db.exec(select(MedicalBed).where(
        MedicalBed.id == id)).first()
